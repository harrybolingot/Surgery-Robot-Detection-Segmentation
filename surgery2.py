...
class SurgeryDataset(utils.Dataset):
    def load_VIA(self, dataset_dir, subset, hc=False):
        ...
        self.add_class("surgery", 1, "endeffector")
        self.add_class("surgery", 2, "peg")
        if hc is True:
            for i in range(1,14):
                self.add_class("surgery", i, "{}".format(i))
            self.add_class("surgery", 14, "endeffector")
        ...
        #   'size': 100202
        # }
        # We mostly care about the x and y coordinates of each regions
        annotations = json.load(open(os.path.join(dataset_dir, "via_regions_data.json")))
        ...
        for a in annotations:
            ...
            # names = [r['regions_attributes'] for r in a['regions'].values()]
            polygons = [r['shape_attributes'] for r in a['regions']]
            names = [r['regions_attributes'] for r in a['regions']]
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        #"name" is the attributes name decided when labeling, etc. 'regions_attributes': {name:'a'}
            if p['name'] == 'blue_peg':
                class_ids[i] = 2
            elif p['name'] == 'red_peg':
                class_ids[i] = 2
            elif p['name'] == 'green_peg':
                class_ids[i] = 2
            elif p['name'] == 'yellow_peg':
                class_ids[i] = 2
            elif p['name'] == 'left_ef':
                class_ids[i] = 1
            elif p['name'] == 'right_ef':
                class_ids[i] = 1
...
def detect_and_color_splash(model, image_path=None, video_path=None, out_dir=''):
    ...
    class_names = ['BG', 'endeffector', 'peg']
